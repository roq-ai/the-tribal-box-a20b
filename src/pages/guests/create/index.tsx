...
function GuestCreatePage() {
  ...
  const formik = useFormik<GuestInterface>({
    initialValues: {
      visited_at: new Date(new Date().toDateString()),
      interest: '',
      location: '',
    ...
    >
      <Box rounded="md">
        ...
        <FormWrapper onSubmit={formik.handleSubmit}>
          <FormControl id="visited_at" mb="4">
            <FormLabel fontSize="1rem" fontWeight={600}>
              Visited At
            </FormLabel>
            <DatePicker
              selected={formik.values?.visited_at ? new Date(formik.values?.visited_at) : null}
              onChange={(value: Date) => formik.setFieldValue('visited_at', value)}
            />
          </FormControl>
