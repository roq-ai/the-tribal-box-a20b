...
import { GuestListPage as ShowCaseList } from 'pages/guests';

function DashboardPage() {
  const { session } = useSession();

  const searchStat = useSWR<PaginatedInterface<SearchInterface>>(
    () => `/searches`,
    () => getSearches({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Searches',
      result: searchStat?.data?.totalCount,
      isLoading: searchStat.isLoading,
